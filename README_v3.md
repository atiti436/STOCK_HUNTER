...
## 🎯 **當前狀態（2026-01-03 20:40）- 下個 Claude 從這裡看起**
...
| `scan_v4.py` | v4.1 | ✅ **完成** | 計分制 + 分批停利 + 門神加強 |
...
### ✅ Phase 2 已完成（2026-01-03）

- [x] **門神加強**（L1095-1098）：`operating_margin < 3`、`gross_margin < 15`
- [x] **推薦數量控制**（L1151-1153）：`score >= 3`，最多 6 檔
- [x] **輸出優化**（L1229-1231）：🔥≥7 / ✅≥5 / 👀≥3
- [x] **測試驗證**：找到 6 檔（國巨 7分、亞光/禾伸堂/緯創 6分、凱美/聯茂 5分）

### ✅ 已解決問題

1. ~~緯創被過濾~~ - TOKEN 重置後已正常
2. ~~營收成功率 3%~~ - 已恢復正常（145/145 成功）
...
### 💾 **資料快取策略（2026-01-03 新增）**

**問題**：Claude 測試邏輯時每次都重新抓 API → 浪費 TOKEN

**解決**：加入 `--offline` 模式 ✅

#### 工作流程
```
GitHub Actions (每天 20:30) → 抓 API → 存到 data/history/
                                    ↓
本地測試 (Claude/你) → git pull → python scan_v4.py --offline → 用快取
```
...
data/
├── raw/                    # 原始資料（145 檔候選股）
│   └── 2026-01-03.json     # ~50KB/天，可重新篩選
├── history/                # 篩選結果（6 檔推薦）
│   └── 2026-01-03.json     # LINE 推送用
```
...
| 階段 | 內容 | 預估時間 | 狀態 |
|------|------|---------|------|
| **Phase 1** | 簡化版 v4.0 | 30 分鐘 | ✅ 已完成 |
| **Phase 2** | 完整版 v4.0 | 1 小時 | ✅ 已完成 |
...
#### 已完成
- [x] 在 final_filter 中呼叫計分函數（L1131-1139）
- [x] 結果加入 `score` 和 `reasons` 欄位（L1145-1147）
- [x] 輸出顯示分數和評分理由（L1229-1235）
- [x] 輸出顯示分批停利（+4%/+7%/+10%）（L1238-1246）
...
## 🔧 2026-01-03 待辦（DON'T PUSH）

> Token 重置了，可以測試

### ✅ 1. v4.1 修正已驗證

**問題**：1/2 收盤價錯誤（150.5 vs 155）
**修正**：欄位名 `Trading_turnover`（已 push）
**驗證**：2026-01-03 21:13 測試通過 ✅
...
**原邏輯**：連續買超（中間斷一天就歸零）
**新邏輯**：5 天內買超天數（允許中間有賣超）
**效果**：技嘉「買買賣買買」= 4 天，不會被歸零
...
目前 scan_v4.py = 1279 行，改東容易漏西

**拆成**：
- `data_loader.py` - 抓資料、清洗
- `strategy.py` - 選股邏輯、計分
- `main.py` - 按鈕人（20 行）

### ❓ 待確認：法人資料抓取邏輯

**疑慮**：不確定 fetch_institutional_history_for_stocks 是否正確
**需要**：印出 raw data 驗證
