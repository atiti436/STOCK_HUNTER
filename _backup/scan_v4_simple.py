...
def filter_and_rank(stocks, pe_data, institutional, historical, revenue, financials):
    ...
    print(f'\n篩選統計:')
    print(f'  無資料: {stats["no_data"]} 檔')
    print(f'  籌碼面不過: {stats["chip_fail"]} 檔 (連續買超<2天 或 5日買超<300張)')
    print(f'  技術面不過: {stats["tech_fail"]} 檔 (5日漲>=10% 或 量<=均量)')
    print(f'  跌破MA20: {stats["ma20_fail"]} 檔')
    print(f'  RSI過熱: {stats["rsi_fail"]} 檔')
    print(f'\n最終通過: {len(results)} 檔')
    return results
