...
def fetch_revenue_data(tickers):
    """
    抓取營收資料並計算 YoY (Backer 付費版批量抓取)
    
    v3.5 改進: 一次抓全市場營收，只需 1 次 API 呼叫！
    ...
    print(f'   [v3.5] 批量抓取全市場營收（1 次 API）...')
    ...
    for attempt in range(max_retries):
        try:
            ...
            # 批量抓取：不指定 stock_id = 抓全市場 (Backer 專屬)
            # 需要抓 400+ 天才能比對去年同月的 YoY
            end_date = datetime.now()
            start_date = end_date - timedelta(days=420)  # 含去年同月
            ...
            print(f'   [OK] 取得全市場營收 ({len(df)} 筆原始資料)')
            ...
            for ticker in tickers_set:
                ticker_df = df[df['stock_id'] == ticker]
                if ticker_df.empty:
                    continue

                # 取最新月份
                ticker_df = ticker_df.sort_values(['revenue_year', 'revenue_month'], ascending=False)
                ...
                # 找去年同期：可能在這次抓的資料裡，也可能沒有
                # 如果沒有，就用單檔抓取補足（但先跳過）
                year_ago = ticker_df[
                    (ticker_df['revenue_month'] == latest_month) &
                    (ticker_df['revenue_year'] == latest_year - 1)
                ]
                ...
                if year_ago.empty:
                    ...
                    # 先嘗試單檔抓取補足
                    try:
                        df_single = dl.taiwan_stock_month_revenue(
                            stock_id=ticker,
                            start_date=(end_date - timedelta(days=400)).strftime('%Y-%m-%d'),
                        )
                        if df_single is not None and not df_single.empty:
                            year_ago = df_single[
                                (df_single['revenue_month'] == latest_month) &
                                (df_single['revenue_year'] == latest_year - 1)
                            ]
                    except:
                        continue
                ...
                year_ago_revenue = float(year_ago.iloc[0]['revenue'])
            ...
            print(f'   [ERROR] 營收批量抓取失敗 (嘗試 {attempt + 1}/{max_retries}): {e}')
            if attempt < max_retries - 1:
                rotate_token()
                time.sleep(1)
            else:
